...
from kfrag.training.stage_d_12bit_transition import transition_loss,transition_metrics,repair_gates,gate_aware_rank
...
def transplant_audit(model,image,bits,tolerance=0.0):
    ...
    return {"passed":all(checks.values()),"checks":checks,"maximum_residual_difference":float((parent["residual"]-r0["residual"]).abs().max().detach().cpu()),"maximum_logit_difference":float((parent["regional_logits"]-r0["packet_logits"][...,4:12]).abs().max().detach().cpu())}
...
def _evaluate(model,images,bits,level,generator):
    ...
    metrics["correct_minus_shuffled_margin"]=metrics["rs_bit_accuracy"]-float(rs_correct.detach().cpu())
    active=tuple(range(4,12))+tuple(range(level));metrics["correct_minus_spatial_margin"]=metrics["overall_active_bit_accuracy"]-float(logits[...,active].ge(0).eq(spatial[...,active].bool()).float().mean().detach().cpu())
    ...
    metrics["original_image_randomized_field_accuracy"]=float(original.ge(0).eq(random_rs.bool()).float().mean().detach().cpu());metrics["original_index_structural_accuracy"]=float(model.index_head(images).ge(0).eq(bits[...,:4].bool()).float().mean().detach().cpu())
    residual=out["residual"];mse=residual.square().flatten(1).mean(1);metrics.update({"psnr":float((10*torch.log10(1/mse.clamp_min(1e-12))).mean().detach().cpu()),"ssim":_ssim(images,out["watermarked_image"]),
      "residual_saturation_fraction":float(residual.abs().ge(.014*.999).float().mean().detach().cpu()),"rs_logit_drift":float((logits[...,4:12]-parent).abs().mean().detach().cpu()),"analytical_contribution":0.0,
    ...
    metrics["cross_region_leakage"]=float(delta[:,1:].mean().detach().cpu())/max(float(delta[:,4].mean().detach().cpu()),1e-12)
    return metrics
...
def run_transition_repair(config:Mapping[str,Any]):
    ...
    trainable=[p for p in model.parameters() if p.requires_grad];maximum=int(config.get("maximum_steps_per_level",300));minimum=int(config.get("minimum_steps_per_level",50));every=int(config.get("evaluate_every",25));batch=int(config.get("batch_size",8));curriculum={"R0":{"passed":True,"step":0,**audit}};history=[];blocked=None;optimizer=None;scheduler=None
    for level in range(1,5):
        optimizer=torch.optim.AdamW(trainable,lr=float(config.get("learning_rate",3e-4)),weight_decay=float(config.get("weight_decay",1e-4)));scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,max(1,maximum));rollback={k:v.detach().clone() for k,v in model.state_dict().items()};best_any=rollback;best_any_score=(-float("inf"),)*4;best_passing=None;best_passing_score=(-float("inf"),)*4;best_metrics=None;terminal_metrics=None;passed=False;stale=0;success_streak=0
        for step in range(1,maximum+1):
            ...
            losses=transition_loss(out["packet_logits"],bits,level,parent_logits,float(config.get("distillation_weight",1)),float(config.get("index_loss_weight",2)))
            ...
            if step%every==0:
                ebits,_=fresh_packet_batch(len(validation),key,generator);metrics=_evaluate(model,validation,ebits,level,generator);terminal_metrics=metrics;gate_pass,score,level_gates=gate_aware_rank(metrics);history.append({"level":f"R{level}","step":step,"gradient_norm_new_before":gb,"gradient_norm_new_after":ga,"new_parameter_update_norm":update,"old_parameter_update_norm":0.0,"selection_gate_results":level_gates,"selection_gate_passed":gate_pass,**metrics})
                if score>best_any_score:best_any_score=score;best_any={k:v.detach().clone() for k,v in model.state_dict().items()};stale=0
                else:stale+=1
                if gate_pass and step>=minimum:
                    success_streak+=1
                    if score>best_passing_score:best_passing_score=score;best_passing={k:v.detach().clone() for k,v in model.state_dict().items()};best_metrics=dict(metrics)
                else:success_streak=0
                if success_streak>=int(config.get("success_evaluations",2)):passed=True;break
                if step>=minimum and stale>=int(config.get("patience_evaluations",10)):break
        selected=best_passing if best_passing is not None else best_any;model.load_state_dict(selected);selected_metrics=best_metrics if best_metrics is not None else terminal_metrics
        curriculum[f"R{level}"]={"passed":passed,"step":step,"selected_from_passing_checkpoint":best_passing is not None,"selected_score":list(best_passing_score if best_passing is not None else best_any_score),"metrics":selected_metrics,"terminal_metrics":terminal_metrics}
