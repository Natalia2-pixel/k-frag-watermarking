...
import math, torch
def communication_metrics(logits,targets):
    ...
    return {"bit_accuracy":float(correct.float().mean().detach()),"per_bit_accuracy":[float(x.detach()) for x in correct.float().mean((0,2,3))],"exact_symbol_accuracy":float(correct[:,4:12].all(1).float().mean().detach()),"exact_tag_accuracy":float(correct[:,12:].all(1).float().mean().detach()),"exact_packet_accuracy":float(correct.all(1).float().mean().detach()),"entropy":float(entropy.mean().detach()),"confidence":float((prob-.5).abs().mul(2).mean().detach()),"predicted_one_frequency":float(pred.float().mean().detach()),"payload_sensitivity":None}
def fidelity_metrics(original,watermarked,residual):
    ...
    return {"psnr":psnr,"ssim":ssim,"lpips":None,"maximum_residual":float(residual.abs().max().detach()),"mean_absolute_residual":float(residual.abs().mean().detach()),"saturation_fraction":None}
